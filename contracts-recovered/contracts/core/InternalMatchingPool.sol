...
contract InternalMatchingPool is ShieldedPool {
    ...
    function _findMatchingIntent(bytes32 intentHash) internal view returns (bytes32 matchHash) {
        ...
        // Generate asset pair hash for indexing
        bytes32 assetPairHash = _getAssetPairHash(intent.inputAssetID, intent.outputAssetID);
     ...
     */
    function _collectMatchedSwaps() internal view returns (bytes32[] memory matchedIds) {
     ...
     */
    function _collectUnmatchedIntents() internal view returns (bytes32[] memory unmatchedHashes) {
